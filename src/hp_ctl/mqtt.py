...
class MqttClient:
    ...
    def __init__(self, broker: str, port: int = 1883, topic_prefix: str = "hp_ctl") -> None:
    ...
    def publish(self, topic: str, payload: dict) -> None:
        ...
        Args:
            topic: Topic name (relative to topic_prefix).
            payload: Dictionary to publish as JSON.
        """
        full_topic = f"{self.topic_prefix}/{topic}"
        json_payload = json.dumps(payload)
        logger.debug("Publishing to %s: %s", full_topic, json_payload)
        self.client.publish(full_topic, json_payload, qos=1)
